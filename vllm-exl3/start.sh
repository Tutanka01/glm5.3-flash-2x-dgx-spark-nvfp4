...
# Per-rank GID override (upstream PR #26). On some 2xGB10 kits the populated
# RoCEv2 GID index differs per node (e.g. 4 on the head, 3 on the worker); a
# single global index then advertises a GID from the wrong subnet on one rank
# and TP init dies with an unhandled NCCL error. Both default to
# NCCL_IB_GID_INDEX, so leaving them unset keeps stock behaviour. docker keeps
# the LAST -e for a repeated key, so these are appended after the shared NCCL
# array in launch_cluster().
...
preflight() {
    ...
    # The GID index each rank uses must name a populated GID on THAT node's
    # CX7 device. An empty (all-zero) entry passes every earlier check and
    # then kills the worker rank ~60 s in with ibv_modify_qp errno 61 "No
    # data available" — kits differ: on some GB10 pairs gid 3 is populated on
    # one node and all-zero on the other. HEAD_GID / WORKER_GID cover kits
    # where no single index is populated on both (upstream PR #26). Fail
    # here, in seconds, with the fix in hand.
    ...
    if [ -z "$gid_head" ] || [ -z "$gid_worker" ]; then
        warn "GID index EMPTY (head ${HEAD_CX7_IB}:${HEAD_GID} $( [ -z "$gid_head" ] && echo 'ALL-ZERO' ) / worker ${WORKER_CX7_IB}:${WORKER_GID} $( [ -z "$gid_worker" ] && echo 'ALL-ZERO' ))"
        warn "GID tables (pick an index whose entry is non-zero — the ::ffff:<ip> RoCEv2 one):"
        for i in 0 1 2 3; do
        ...
        die "set NCCL_IB_GID_INDEX — or per-rank HEAD_GID / WORKER_GID — in .env to a populated index (this kills the rank with ibv_modify_qp errno 61 otherwise)"
    fi
