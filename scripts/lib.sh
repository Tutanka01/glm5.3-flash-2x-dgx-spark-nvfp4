...
glm53_load_config() {
  ...
  # A profile file owns these experimental selectors. Reset them after the
  # shared env so a previous glm53_load_config call cannot leak into another
  # profile.
  PROFILE_TIER="validated"
  PROFILE_RUNTIME_IMAGE=""
  SPECULATIVE_ALGORITHM=""
  DFLASH_DRAFT_MODEL_PATH=""
  DFLASH_DRAFT_ATTENTION_BACKEND=""
  ...
  RUNTIME_IMAGE_EFFECTIVE="${PROFILE_RUNTIME_IMAGE:-$GLM53_RUNTIME_IMAGE}"
  ...
  export PROFILE_TIER PROFILE_RUNTIME_IMAGE RUNTIME_IMAGE_EFFECTIVE
  export SPECULATIVE_ALGORITHM DFLASH_DRAFT_MODEL_PATH
  export DFLASH_DRAFT_ATTENTION_BACKEND DFLASH_DRAFT_WINDOW_SIZE
  export MAMBA_SSM_DTYPE MAX_MAMBA_CACHE_SIZE
...
glm53_worker_compose() {
  ...
  command_parts=(
    ...
    "PROFILE_RUNTIME_IMAGE=$PROFILE_RUNTIME_IMAGE"
    "SPECULATIVE_ALGORITHM=$SPECULATIVE_ALGORITHM"
    "DFLASH_DRAFT_MODEL_PATH=$DFLASH_DRAFT_MODEL_PATH"
    "DFLASH_DRAFT_ATTENTION_BACKEND=$DFLASH_DRAFT_ATTENTION_BACKEND"
...
glm53_sync_worker() {
  ...
  if [ -d "$GLM53_ROOT/dflash2" ]; then
    glm53_scp -r "$GLM53_ROOT/dflash2" "$WORKER_HOST:$WORKER_DIR/"
  fi
