...
if "$ROOT_DIR/status-glm53.sh" 256k-mtp > "$TEST_STATE/unsafe-status" 2>&1; then
  printf 'unsafe 256k-mtp status unexpectedly passed\n' >&2
  exit 1
fi
grep -F 'Cold >128k preflight: REFUSED' "$TEST_STATE/unsafe-status" >/dev/null
